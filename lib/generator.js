 ...
 * @constructor
 */
const Dungeon = function Dungeon(options) {
  var numRoomTries = options.numRoomTries || 50;
  
    // The inverse chance of adding a connector between two regions that have
    // already been joined. Increasing this leads to more loosely connected
    // dungeons.
    var extraConnectorChance = options.extraConnectorChance || 50;
  
    // Increasing this allows rooms to be larger.
    var roomExtraSize = options.roomExtraSize || 0;
  
    var windingPercent = options.windingPercent || 70;
...
const generate = (options) => {
  let d = new Dungeon(options)
  return d.generate(options);
};
